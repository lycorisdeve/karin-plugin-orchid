import { render, karin, segment, logger } from 'node-karin';
import { fetchRSS } from '../rss/rss.js';
import { config } from '../utils/config.js';
import { dirPath, } from '../utils/dir.js';
import { collectHandlers } from '../rss/post_handler.js';
import { formatRssPubDate } from '../utils/common.js';
import { buildRssUrl, DEFAULT_RSS_HUB_BASEURL } from '../rss/presets/index.js';
let rssLock = false;
            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (const rssHubUrl of rssHubUrls) {
                    try {
                        ...
                        let url = rss.url?.replaceAll('${rsshub_url}', rssHubUrl);
                        const postHandlers = rss.postHandlers || [];
                        if (rss.preset) {
                            const rssPreset = buildRssUrl(rss.presetConfig, rss.preset, rssHubUrl);
                        ...
                        logger.error(`Error fetching RSS with url ${rssHubUrl}:`, e);
                        continue;
                    }
                }
                logger.error(`All RSSHub URLs failed for ${rss.name}`);
                resolve();
            });
        });
