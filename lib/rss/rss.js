...
import { config, DATA_DIR } from '../utils/config.js';
 ...
 * @param options
 */
export const fetchRSS = async (subscribeName, url, options) => {
    ...
    if (config().proxy) {
        fetchOption.agent = new HttpsProxyAgent(config().proxy);
    ...
    let latestItem;
    let allItems = storedData.allItems;
    for (const item of feed.items.sort((a, b) => new Date(a.pubDate).getTime() - new Date(b.pubDate).getTime())) {
        if (!item.pubDate) {
            // 比如github star没有时间
            // 只能靠link来全存然后筛选了罢！
